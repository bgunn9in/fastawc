...
### Windows Native `big.txt` Comparison

Measured on Windows 11 against a 3.2 GiB `big.txt` sample on the same `E:` filesystem. The table uses external wall-clock subprocess time without `fastawc --speed`, 10 warmup runs, and 5 measured runs per command. GNU `wc` is Git for Windows coreutils 8.32 with `LC_ALL=C.UTF-8`; the UTF-8 locale is required for `wc -m`, `wc -w`, and `wc -L` to match Unicode text semantics.

Classic `-l -w -c`:

| command | avg time | throughput | vs `wc` |
|---|---:|---:|---:|
| `fastawc --fast -l -w -c big.txt` | `0.451s` | `7097.1 MiB/s` | `35.72x` faster |
| `fastawc --strict -l -w -c big.txt` | `0.650s` | `4926.1 MiB/s` | `24.79x` faster |
| `LC_ALL=C.UTF-8 wc -l -w -c big.txt` | `16.124s` | `198.7 MiB/s` | baseline |

Full `-l -w -c -m -L`:

| command | avg time | throughput | vs `wc` |
|---|---:|---:|---:|
| `fastawc --fast -l -w -c -m -L big.txt` | `0.496s` | `6457.7 MiB/s` | `33.00x` faster |
| `fastawc --strict -l -w -c -m -L big.txt` | `0.932s` | `3438.0 MiB/s` | `17.57x` faster |
| `LC_ALL=C.UTF-8 wc -l -w -c -m -L big.txt` | `16.371s` | `195.7 MiB/s` | baseline |
